...
public record Region(Coordinate startIncl, Coordinate endIncl) {
    ...
    public ImmutableSet<Coordinate> getPerimeter() {
        ImmutableSet.Builder<Coordinate> builder = ImmutableSet.builder();
        for (int x = startIncl.x() - 1; x < getWidth() + startIncl().x() + 1; x++) {
            builder.add(new Coordinate(x, startIncl.y() - 1));
            builder.add(new Coordinate(x + 1, endIncl.y() + 1));
        }
        for (int y = startIncl.y() - 1; y < getHeight() + startIncl().y() + 1; y++) {
            builder.add(new Coordinate(startIncl.x() - 1, y + 1));
            builder.add(new Coordinate(endIncl.x() + 1, y));
        }
    ...
    private enum Direction {
        ...
        public Coordinate pushCoordinate(Coordinate start, int amount) {
